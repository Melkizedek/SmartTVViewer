...
public class ChildManagement {
    ...
    public static void setRestrictions(Calendar fromTime, Calendar toTime,
	    long maxTime, ArrayList<TVChannel> channels) {
	...
	for(int i = 0; i < channelsString.size(); i++) {
		...
		if(Initializer.tvChannelList.get(j).toString()
			.equals(channelsString.get(i).toString())) {
	...
	if(c != null
		&& c.get(Calendar.DAY_OF_WEEK) == today
