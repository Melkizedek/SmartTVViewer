...
public class TVSeriesView {
	...
	for(int i = 0; i < Initializer.tvChannelList.size(); i++){
	    for(int j = 0; j < Initializer.tvChannelList.get(i).getBroadcasts().size(); j++){
		listModel.addElement(Initializer.tvChannelList.get(i).getBroadcasts().get(j));
	    }
	}
	...
	JLabel lblMinutesBeforeThe = new JLabel("minutes before the Show starts");
